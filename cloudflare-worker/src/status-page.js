...
function checkMethod(server) {
  ...
  if (value.includes('tcp')) return 'TCP 端口';
  if (value.includes('http')) return 'HTTP(S)';
  return '魔方财务 API';
}
...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}body{margin:0;min-height:100vh;background:radial-gradient(circle at 12% 0,rgba(16,201,143,.12),transparent 28%),linear-gradient(180deg,#fff,var(--bg));color:var(--ink);font-family:"Bahnschrift","Aptos Display","Microsoft YaHei UI",sans-serif}
    main{width:min(900px,calc(100% - 36px));margin:0 auto;padding:46px 0 64px}.pageNav{display:flex;justify-content:flex-end;margin-bottom:20px}.adminLink{border:1px solid var(--line);background:#fff;color:#0f1b2d;text-decoration:none;border-radius:999px;padding:11px 18px;font-weight:800;box-shadow:0 14px 30px rgba(15,27,45,.09)}
    .hero{display:flex;align-items:end;justify-content:space-between;gap:18px;margin-bottom:30px}.tag{color:#0b9f75;letter-spacing:.18em;font-size:12px;font-weight:900;text-transform:uppercase}h1{font-size:38px;margin:10px 0 6px;letter-spacing:-.05em}.lead{max-width:780px;color:var(--muted);line-height:1.65;margin:0}
    .service-title{font-size:28px;margin:22px 0 12px}.group-title{font-size:24px;margin:0 0 12px}.grid{display:grid;gap:16px;width:min(100%,680px)}.status-card{min-width:0;border:1px solid #cfd9e8;background:linear-gradient(180deg,#fff,#f9fbff);box-shadow:0 20px 48px rgba(15,27,45,.10);border-radius:22px;padding:20px;animation:rise .45s ease both}.status-card--healthy{border-color:#b9e9d9}.status-card--down,.status-card--recovering,.status-card--rebooting{border-color:#ffc4cc}
